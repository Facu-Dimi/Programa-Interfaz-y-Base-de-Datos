...
# Clase Login contiene la ventana de inicio de sesion al programa
...
# Clase Frame (Ventana principal del programa)
...
class Frame(tk.Frame):
    ...
    def campos_pedido(self):
        ...
        self.boton_modificar.config(width = 20, font = ('Arial', 11, 'bold'), fg = 'snow', bg = 'RoyalBlue',
                                cursor = 'hand2', activebackground = 'slate blue')
        ...
        self.boton_eliminar.config(width = 20, font = ('Arial', 11, 'bold'), fg = 'snow', bg = 'RoyalBlue',
                                cursor = 'hand2', activebackground = 'slate blue')
        self.boton_eliminar.grid(row = 9, columnspan = 2, column = 0, padx = 10, pady = 10)

    
    # Treeview, la tabla que contiene los valores de la base de datos.


    def tabla_datos(self):
        self.tabla = ttk.Treeview(self, column = ('Alumno', 'Curso', 'Profesor', 'Fecha', 'Horario de Entrega', 'Horario de Devolucion'))
        ...
        self.tabla.heading('#5', text = 'Entrega')
        self.tabla.heading('#6', text = 'Devolucion')
        
    
    # Metodos que ejecutan los botones.
    ...
    def registrar_datos(self):
        try:
            ...
            curso = self.base_datos.obtener_curso(alumno)[0][0]
            profesor = self.profesor.get()
            #entrega = self.entrega.get()
            devolucion = self.devolucion.get()
            if numero and alumno and curso and devolucion !='':
                if numero == self.pedido_edit:
                    self.tabla.insert('',0, text = numero, values=datos)
                    self.base_datos.editar_pedido(numero, alumno, devolucion)
                else:      
                    self.base_datos.registrar_pedido(numero, alumno, profesor, devolucion)
                    fecha = self.base_datos.obtener_fecha(numero)[0][0]
                    entrega = self.base_datos.obtener_fecha(numero)[0][1]
                    datos = (alumno, curso, profesor, fecha, entrega, devolucion)
                    self.tabla.insert('',0, text = numero, values=datos)
                    self.limpiar_campos()
        except:
            self.limpiar_campos()
    ...
    def mostrar_datos(self):
        ...
        for dato in registro:
            i= i+1                       
            self.tabla.insert('',i, text = registro[i][0:1], values= registro[i][1:7])
    

    
    # Modificar y eliminar_datos por el momento no funcionan, tengo que reprogramarlos y adaptarlos a la base de datos de alexis.
    
    
    def modificar_datos(self):
        numero = self.tabla.item(self.tabla.selection())['text']
        alumno = self.tabla.item(self.tabla.selection())['values'][0]
        curso = self.tabla.item(self.tabla.selection())['values'][1]
        profesor = self.tabla.item(self.tabla.selection())['values'][2]
        entrega = self.tabla.item(self.tabla.selection())['values'][3]
        devolucion = self.tabla.item(self.tabla.selection())['values'][4]
        
        self.pedido_edit = numero
        
        self.entry_numero.insert(0, numero)
        self.entry_alumno.insert(0, alumno)
        self.entry_curso.insert(0, curso)
        self.entry_profesor.insert(0, profesor)
        self.entry_entrega.insert(0, entrega)
        self.entry_devolucion.insert(0, devolucion)
